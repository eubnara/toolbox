...
Samsung의 클립보드 기록은 **최대 약 50개** 항목을 유지합니다. 이 앱은 50개의 더미 클립(`.0`, `.1`, `.2`, ...)을 연속으로 주입(flood)하여 기존 기록을 밀어내는 방식으로 동작합니다.

1. `ClipboardManager.setPrimaryClip()`을 50회 호출
...
- **수동 플러드**: 버튼 한 번으로 50개 더미 클립 즉시 주입
